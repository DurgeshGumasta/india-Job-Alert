...
export const run: ActionRun = async ({ params, logger, connections }) => {
  ...
  const regexResult = pdfData.text// extractWithRegex(pdfData.text);
  
  logger.info(`Regex extraction completed. Fields extracted: ${Object.keys(regexResult)}, Complete: ${regexResult.isComplete}`);
